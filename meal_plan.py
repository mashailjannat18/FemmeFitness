...
def calculate_height_cm(height_feet):
    feet = int(height_feet)
    inches = (height_feet - feet) * 10
    height_cm = (feet * 30.48) + (inches * 2.54)
    return height_cm

def load_user_profile(data: Dict[str, Any], cycle_phases: List[Dict]) -> None:
    global USER_AGE, USER_AGE_GROUP, USER_ACTIVITY_LEVEL, USER_GOAL, USER_WEIGHT, USER_HEIGHT,USER_HEIGHT_CM, USER_BMI, USER_HEALTH_CONDITIONS, USER_CYCLE_PHASES
    try:
        ...
        USER_HEIGHT_CM = calculate_height_cm(USER_HEIGHT)
...
def adjust_calories(age: int, weight_kg: float,height_cm: float,activity_level: str) -> float:
...
def adjust_meal_calories(base_calories: float, calories_burned: float, goal: str) -> float:
    if goal == "weight_loss":
        calories = max(1200, base_calories - 600 + calories_burned)
...
def get_cycle_phase_for_day(day_number: int, workout_plan: List[Dict]) -> str:
    for day_data in workout_plan:
        if int(day_data['Day'].split('(')[0].replace('Day ', '').strip()) == day_number:
            return day_data.get('Cycle Phase', 'follicular')
...
def generate_meal_plan(workout_plan_data: Tuple[List[Dict], str], data: Dict[str, Any], cycle_phases: List[Dict] = None) -> List[Dict]:
    try:
        ...
        for day_data in workout_plan:
            ...
            cycle_phase = get_cycle_phase_for_day(day_number, workout_plan)
            ...
            base_calories = adjust_calories(USER_AGE, USER_WEIGHT, USER_HEIGHT_CM, USER_ACTIVITY_LEVEL)
